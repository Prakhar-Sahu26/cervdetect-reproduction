...
equal to the majority-class baseline. The all-negative collapse is the modal outcome across random initialisations,
though qualitatively different collapse modes are also observed. The result
holds across every interpretation of the ambiguous implementation details that
we tested.
...
used \texttt{RandomForestClassifier} with \texttt{n\_estimators=100} and
\texttt{random\_state=42}, otherwise default settings, fitted to the complete
$858 \times 32$ frame with Biopsy as the target.
...
Omitting this step has a measurable effect on the optimisation regime. The input variables differ
...
to $0.0968$---a factor of approximately 9.5. Without normalisation the network begins training with approximately 90\% of
hidden units in the saturated region and a mean sigmoid derivative
approximately 9.5-fold smaller than with scaled inputs; this is consistent
with the optimisation difficulty we observe in its absence
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{loss_trajectory.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{seed_sweep.png}
...
The stochastic components are seeded explicitly. The primary Random Forest is
fitted with a fixed seed, while the data partition and the neural-network
...
and the scripts that generate each figure, is available in the repository cited
above. The dataset is not redistributed and is obtained directly from the UCI
repository.
...
which shows nine. The six-epoch interval is consistent with the stopping rule
adopted here, but does not establish how epochs were counted in the original
implementation.
...
and an unexamined default threshold has been documented as a source of
misleading evaluation in imbalanced-classification literature, and it produces
results that are simultaneously reproducible and uninformative. The corrective is inexpensive: a baseline
...
accuracy figure suggests. A model with sensitivity zero provides no benefit
over a rule that assigns every record to the negative class, regardless of its
headline accuracy.
...
better. That variant is not itself a reproduction of the original, which
specifies the leaky ordering.
